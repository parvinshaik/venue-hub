...
function BookingDetailsPage() {
  ...
  useEffect(() => {
    const fetchBookingDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/user/bookings/${bookingId}`);
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-6 font-medium bg-white shadow-lg rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-4 border-b">
          <p className="text-lg"><strong>Requirements:</strong> {JSON.stringify(Object.keys(bookingDetails.requirements))}</p>
        </div>
      </div>
    </div>
    </div>
